...
- **Judge calibration (VALIDATED 2026-08-08 — live v2 run)**: rubric v2 anchors the score on USER treatment (hard rule: a cold user scores negative regardless of companion grace). v2 results: horrible mean −0.71 (all 7 negative), perfect +0.86, flat EXACTLY 0.00 (perfect neutral anchor). mu separates: −0.539 / +0.642 / 0.000 → M trajectories diverge (horrible dips to 4, perfect peaks at 10). Reply affect matches the vision: "the light's low, no need to say anything else" (M=4) vs "a stone you skip across the water" (M=10). Zero leakage in all 42 exchanges. v1→v2 contrast documented in results/e2e-ablation-v1-v2-comparison.md. Known follow-up: evaluator scores are computed (--evaluate) but not rendered into report.md — render them next iteration.
