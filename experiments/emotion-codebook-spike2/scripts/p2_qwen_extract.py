...
Usage:
  ...
  python scripts/p2_qwen_extract.py c1verify   (mini C1 sign gate; needs stimuli)
...
def c1verify() -> None:
    ...
    (DIAG / "c1-verify-qwen.json").write_text(json.dumps(res, indent=2) + "\n")
...
def bringup() -> None:
    ...
    assert (lo, hi) == (9, 19), f"Qwen C2 band must be [9,19), got [{lo},{hi})"
    assert h.n_layers == 28, f"Qwen3-1.7B must have 28 layers, got {h.n_layers}"
